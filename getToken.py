...
Tokenurl = "https://lzkj-isv.isvjcloud.com"

# 代理相关
proxyUrl = "http://api2.xkdaili.com/tools/XApi.ashx?apikey=XKA4441598B79E725C22&qty=1&format=txt&split=0&sign=6a177f8327cee5b0162972e8dab7b086&s5=3"  # 代理API 使用socks5 \n分割 只能提取一个
...
# 环境redis换成
redis_url = "redis://:192.168.50.200:6379"  # "redis://:Redis密码@127.0.0.1:6379" 默认读取 PRO_REDIS_URL 变量的
...
if os.environ.get("JD_ISV_TOKEN_CUSTOM_CACHE", None) is not None:
    ...
    if CiShanJiaToken == KRCacheToken:
        CiShanJiaToken = None
class Proxy:
    def __init__(self, url=None, user_pass=None, redis_url=None):
        self.proxies = None
        self.url = url
...
proxy = Proxy(url=proxyUrl, user_pass=user_pass, redis_url=redis_url)


async def fetch_url_with_proxy(url, headers=None, data=None, proxy=None):
    try:
        async with aiohttp.ClientSession(connector=TCPConnector(ssl=False)) as session:
            async with session.post(url=url, headers=headers, data=data, proxy=proxy, ssl=False) as response:
                return {
...
async def main_async():
    token_json = [KRCacheToken, CiShanJiaToken]
    KR = {}
    # 获取代理
    await proxy.getproxy()
    # 链接redis
